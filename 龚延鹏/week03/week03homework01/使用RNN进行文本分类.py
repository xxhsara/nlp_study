...
dataset = pd.read_csv("D:\BaiduNetdiskDownload\Week01-课程介绍与大模型基础\Week01\dataset.csv", sep="\t", header=None)
...
class RNNClassifier(nn.Module):
    ...
    #     embedded = self.embedding(x)
    #     rnn_out, hidden_state = self.rnn(embedded)
    #     # 排查：打印 hidden_state 的类型和形状
    #     print("hidden_state 类型：", type(hidden_state))
    #     print("hidden_state 形状：", hidden_state.shape)
    #     out = self.fc(hidden_state.squeeze(0))
    #     return out
