...
class CharGRUDataset(Dataset):
    ...
    def __getitem__(self, id):
        ...
        return torch.tensor(indices, dtype=torch.long), self.labels[id]
...
class GRUClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        gru_out, last = self.gru(embedded)

        out = self.fc(last.squeeze(0))
        return out
...
for epoch in range(num_epochs):
    ...
    print(f' RNN 第 {epoch + 1} 轮训练完成 Loss [{running_loss / len(rnn_dataloader) : .4f}]')
    ...
    print(f' GRU 第 {epoch + 1} 轮训练完成 Loss [{running_loss / len(gru_dataloader) : .4f}]')

    lstm_model.train()
    running_loss = 0.0
    for idx, (inputs, labels) in enumerate(lstm_dataloader):
    ...
    print(f' LSTM 第 {epoch + 1} 轮训练完成 Loss [{running_loss / len(lstm_dataloader) : .4f}]')
